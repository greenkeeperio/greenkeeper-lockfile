...
module.exports = function updateLockfile (dependency, options) {
  ...
  // TODO(patkub): detect AppVeyor and commit using Windows
  ...
  */
  // AppVeyor commit the updated lockfile
  // Note: (exit 0), the parentheses create a subshell that exits with status 0, instead of exiting the current shell
  exec('git add npm-shrinkwrap.json || (exit 0)')
