...
module.exports.updateLockfile = function updateLockfile (dependency, options) {
  if (!options.pnpm && !options.yarn && semver.lt(exec('npm --version').toString().trim(), '3.0.0')) {
    exec('npm shrinkwrap')
  } else {
    if (options.pnpm) {
      const flag = pnpmFlags[dependency.type]
      const args = `${flag} --lockfile-only '${dependency.name}@${dependency.range}'`
...
module.exports.stageLockfile = function stageLockfile (options) {
  ...
  exec(`git add pnpm-lock.yaml ${ignoreOutput}`)
}
